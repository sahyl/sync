...
const MeetingSetup = ({
  ...
  const [isMikeCamToggledOn, setIsMikeCamToggledOn] = useState(false);
  const call = useCall();
  if (!call) {
    throw new Error("useCall must be  used within StreamCall component");
  }
  useEffect(() => {
    if (isMikeCamToggledOn) {
      call?.camera.disable();
  ...
  }, [isMikeCamToggledOn, call?.camera, call?.microphone]);
  return (
